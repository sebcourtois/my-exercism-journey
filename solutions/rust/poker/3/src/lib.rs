...
impl Hand {
    ...
    pub fn a_straight(&self) -> bool {
        let highest_rank = self.sorted_cards.last().unwrap().rank as i8;
        let lowest_rank = self.sorted_cards.first().unwrap().rank as i8;
...
pub fn winning_hands<'a>(hands: &[&'a str]) -> Vec<&'a str> {
    let ranked_hands: Vec<Hand> = hands.iter()
        .map(|h| Hand::from_string(h))
        .collect();

    let mut hands_by_ranking: HashMap<u32, Vec<&str>> = HashMap::new();
    for hand in ranked_hands.iter() {
        let ranking = hand.ranking();
        hands_by_ranking
            .entry(ranking)
            .or_default()
            .push(hand.display_string.as_str());
